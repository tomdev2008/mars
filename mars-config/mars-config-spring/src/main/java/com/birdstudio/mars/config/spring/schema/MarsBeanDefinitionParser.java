...
import com.birdstudio.mars.config.ArgumentConfig;
import com.birdstudio.mars.config.ConsumerConfig;
import com.birdstudio.mars.config.MethodConfig;
...
public class MarsBeanDefinitionParser implements BeanDefinitionParser {
	...
	private static BeanDefinition parse(Element element,
		...
		// 设置id值，次序依次是id,name,interface,类名,如果是protocal元素默认是mars
		...
		if ((id == null || id.length() == 0) && required) {
			...
			if (generatedBeanName == null || generatedBeanName.length() == 0) {
				if (ProtocolConfig.class.equals(beanClass)) {
					generatedBeanName = "mars";
				} else {
					generatedBeanName = element.getAttribute("interface");
				}
			}
			// 如果generatedBeanName还没值，直接用Class的值赋值
			if (generatedBeanName == null || generatedBeanName.length() == 0) {
		...
		if (ProtocolConfig.class.equals(beanClass)) {
			for (String name : parserContext.getRegistry()
				...
				// 如果其它元素有相同protocol值，把它设为RuntimeBeanReference
				if (property != null) {
				...
				if ("parameters".equals(property)) {
					...
					if (StringUtils.isNotEmpty(value)) {
						if ("registry".equals(property)) {
							if (RegistryConfig.NO_AVAILABLE
								...
								parseMultiRef("registries", value,
							...
							if (isPrimitive(type)) {
								if ("async".equals(property)
										&& "false".equals(value)
										|| "timeout".equals(property)
										&& "0".equals(value)
										|| "delay".equals(property)
										&& "0".equals(value)
										|| "version".equals(property)
										&& "0.0.0".equals(value)
										|| "stat".equals(property)
										&& "-1".equals(value)
								...
								reference = value;
									...
									&& ExtensionLoader.getExtensionLoader(
											Protocol.class).hasExtension(value)
									&& (!parserContext.getRegistry()
											.containsBeanDefinition(value) || !ProtocolConfig.class
											.getName()
											.equals(parserContext.getRegistry()
													.getBeanDefinition(value)
													.getBeanClassName()))) {
								if ("mars:provider"
										.equals(element.getTagName())) {
									logger.warn("Recommended replace <mars:provider protocol=\""
											+ value
											+ "\" ... /> to <mars:protocol name=\""
											+ value + "\" ... />");
								}
								// 兼容旧版本配置
								...
								reference = protocol;
									...
									&& (!parserContext.getRegistry()
											.containsBeanDefinition(value) || !MonitorConfig.class
											.getName()
											.equals(parserContext.getRegistry()
													.getBeanDefinition(value)
													.getBeanClassName()))) {
								...
								String returnMethod = value
										.substring(index + 1);
								reference = new RuntimeBeanReference(returnRef);
								beanDefinition.getPropertyValues()
										.addPropertyValue("onreturnMethod",
												returnMethod);
								...
								beanDefinition.getPropertyValues()
										.addPropertyValue("onthrowMethod",
												throwMethod);
							} else {
								if ("ref".equals(property)
										&& parserContext.getRegistry()
												.containsBeanDefinition(value)) {
									BeanDefinition refBean = parserContext
											.getRegistry().getBeanDefinition(
													value);
									if (!refBean.isSingleton()) {
										throw new IllegalStateException(
												"The exported service ref "
														+ value
														+ " must be singleton! Please set the "
														+ value
														+ " bean scope to singleton, eg: <bean id=\""
														+ value
														+ "\" scope=\"singleton\" ...>");
									}
								}
	...
	private static final Pattern GROUP_AND_VERION = Pattern
	...
	private static void parseNested(Element element,
			ParserContext parserContext, Class<?> beanClass, boolean required,
			String tag, String property, String ref,
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				if (node instanceof Element) {
					if (tag.equals(node.getNodeName())
						...
						BeanDefinition subDefinition = parse((Element) node,
								parserContext, beanClass, required);
						if (subDefinition != null && ref != null
								&& ref.length() > 0) {
							subDefinition.getPropertyValues().addPropertyValue(
									property, new RuntimeBeanReference(ref));
						}
					}
				}
			}
	 ...
	 */
	private static void parseProperties(NodeList nodeList,
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				if (node instanceof Element) {
					if ("parameter".equals(node.getNodeName())
						...
						boolean hide = "true".equals(((Element) node)
								.getAttribute("hide"));
						if (hide) {
	...
	private static void parseMethods(String id, NodeList nodeList,
		...
		if (nodeList != null && nodeList.getLength() > 0) {
				...
				if (node instanceof Element) {
					...
					if ("method".equals(node.getNodeName())
						...
						if (methodName == null || methodName.length() == 0) {
							throw new IllegalStateException(
									"<mars:method> name attribute == null");
						}
						if (methods == null) {
							methods = new ManagedList();
						}
						BeanDefinition methodBeanDefinition = parse(
								((Element) node), parserContext,
			...
			if (methods != null) {
				beanDefinition.getPropertyValues().addPropertyValue("methods",
						methods);
			}
		}
	}
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				if (node instanceof Element) {
					...
					if ("argument".equals(node.getNodeName())
						...
						BeanDefinition argumentBeanDefinition = parse(
								((Element) node), parserContext,
