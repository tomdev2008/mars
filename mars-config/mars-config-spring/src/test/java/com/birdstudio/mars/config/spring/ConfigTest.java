...
import com.birdstudio.mars.config.ReferenceConfig;
...
public class ConfigTest {
	...
	// ctx.start();
	// try {
	// MockFilter filter = (MockFilter) ExtensionLoader
	// .getExtensionLoader(Filter.class).getExtension("mymock");
	...
	public void testServiceClass() {
		...
		ctx.start();
		try {
			DemoService demoService = refer("mars://127.0.0.1:20887");
	...
	// ctx.start();
	// try {
	// ServiceConfig<DemoService> serviceConfig = (ServiceConfig<DemoService>)
	// ctx
	// .getBean("serviceConfig");
	// assertNotNull(serviceConfig.getProvider());
	// assertEquals(2000, serviceConfig.getProvider().getTimeout()
	// .intValue());
	//
	// ServiceConfig<DemoService> serviceConfig2 = (ServiceConfig<DemoService>)
	// ctx
	// .getBean("serviceConfig2");
	// assertNotNull(serviceConfig2.getProvider());
	// assertEquals(1000, serviceConfig2.getProvider().getTimeout()
	// .intValue());
	// } finally {
	// ctx.stop();
	// ctx.close();
	...
	// assertTrue(str
	// .contains(" interface=\"com.alibaba.dubbo.config.spring.api.DemoService\" "));
	...
	// Exporter<RegistryService> exporter = SimpleRegistryExporter.export(
	// 4547, registryService);
	// ClassPathXmlApplicationContext ctx = new ClassPathXmlApplicationContext(
	// ConfigTest.class.getPackage().getName().replace('.', '/')
	// + "/multi-protocol-register.xml");
	// ctx.start();
	// try {
	// List<URL> urls = registryService.getRegistered().get(
	// "com.alibaba.dubbo.config.spring.api.DemoService");
	// assertNotNull(urls);
	// assertEquals(1, urls.size());
	// assertEquals("dubbo://" + NetUtils.getLocalHost()
	// + ":20824/com.alibaba.dubbo.config.spring.api.DemoService",
	...
	// Exporter<RegistryService> exporter1 = SimpleRegistryExporter.export(
	// 4545, registryService1);
	// SimpleRegistryService registryService2 = new SimpleRegistryService();
	// Exporter<RegistryService> exporter2 = SimpleRegistryExporter.export(
	// 4546, registryService2);
	// ClassPathXmlApplicationContext ctx = new ClassPathXmlApplicationContext(
	// ConfigTest.class.getPackage().getName().replace('.', '/')
	// + "/multi-registry.xml");
	// ctx.start();
	// try {
	// List<URL> urls1 = registryService1.getRegistered().get(
	// "com.alibaba.dubbo.config.spring.api.DemoService");
	// assertNull(urls1);
	// List<URL> urls2 = registryService2.getRegistered().get(
	// "com.alibaba.dubbo.config.spring.api.DemoService");
	// assertNotNull(urls2);
	// assertEquals(1, urls2.size());
	// assertEquals("dubbo://" + NetUtils.getLocalHost()
	// + ":20880/com.alibaba.dubbo.config.spring.api.DemoService",
	...
	// urls = registryService.getRegistered().get(
	// "com.alibaba.dubbo.config.spring.api.DemoService");
	// Thread.sleep(10);
	// }
	// assertNotNull(urls);
	...
	// try {
	// ClassPathXmlApplicationContext byNameContext = new
	// ClassPathXmlApplicationContext(
	// ConfigTest.class.getPackage().getName().replace('.', '/')
	// + "/aop-autowire-byname.xml");
	// byNameContext.start();
	// try {
	// DemoActionBySetter demoActionBySetter = (DemoActionBySetter)
	// byNameContext
	// .getBean("demoActionBySetter");
	// assertNotNull(demoActionBySetter.getDemoService());
	// assertEquals("aop:say:hello", demoActionBySetter
	// .getDemoService().sayName("hello"));
	// DemoActionByAnnotation demoActionByAnnotation = (DemoActionByAnnotation)
	// byNameContext
	// .getBean("demoActionByAnnotation");
	// assertNotNull(demoActionByAnnotation.getDemoService());
	// assertEquals("aop:say:hello", demoActionByAnnotation
	// .getDemoService().sayName("hello"));
	// } finally {
	// byNameContext.stop();
	// byNameContext.close();
	// }
	// ClassPathXmlApplicationContext byTypeContext = new
	// ClassPathXmlApplicationContext(
	// ConfigTest.class.getPackage().getName().replace('.', '/')
	...
	// assertNotNull(urls);
	// assertEquals(1, urls.size());
	// assertEquals("classloader,monitor,accesslog,trace", urls.get(0)
	// .getParameter("service.filter"));
	//
	// ConsumerConfig consumer = new ConsumerConfig();
	...
	// reference.setInterface(DemoService.class);
	// reference.setUrl("dubbo://" + NetUtils.getLocalHost() + ":20880?"
	// + DemoService.class.getName() + "?check=false");
	// try {
	// reference.get();
	...
	// assertThat(
	// expected.getMessage(),
	// containsString("must implement java.io.Serializable"));
	// }
	// } finally {
	// ctx.stop();
	// ctx.close();
	...
	// providerContext.start();
	// try {
	// ApplicationConfig application = (ApplicationConfig) providerContext
	// .getBean("application");
	// assertEquals("demo-provider", application.getName());
	// assertEquals("world", application.getOwner());
	//
	// RegistryConfig registry = (RegistryConfig) providerContext
	// .getBean("registry");
	// assertEquals("N/A", registry.getAddress());
	//
	// ProtocolConfig dubbo = (ProtocolConfig) providerContext
	// .getBean("dubbo");
	...
	// assertEquals("api-override-properties",
	...
	// ProtocolConfig rmi = (ProtocolConfig) providerContext
	// .getBean("rmi");
	...
	// providerContext.start();
	// try {
	// ServiceConfig<DemoService> service = (ServiceConfig<DemoService>)
	// providerContext
	...
	//
	// // @Test
	// // public void testSystemPropertyOverrideReferenceConfig() throws
	// Exception
	// // {
	...
	// try {
	// assertEquals("sysover", serviceConfig.getApplication()
	// .getName());
	// assertEquals("sysowner", serviceConfig.getApplication()
	// .getOwner());
	// assertEquals("N/A", serviceConfig.getRegistry().getAddress());
	// assertEquals("dubbo", serviceConfig.getProtocol().getName());
	// assertEquals(20834, serviceConfig.getProtocol().getPort()
	// .intValue());
	// } finally {
	...
	// context.start();
	// ServiceBean<DemoService> serviceBean = (ServiceBean<DemoService>) context
	// .getBean("demoServiceExport");
	// URL url = serviceBean.toUrls().get(0);
	...
	// ClassPathXmlApplicationContext(
	// ConfigTest.class.getPackage().getName()
	// .replace('.', '/')
	// + "/annotation-consumer.xml");
	// consumerContext.start();
	// try {
	// AnnotationAction annotationAction = (AnnotationAction) consumerContext
	...
	// service.export();
	//
	// Assert.assertEquals(port, service.getExportedUrls().get(0)
	// .getPort());
	// } finally {
