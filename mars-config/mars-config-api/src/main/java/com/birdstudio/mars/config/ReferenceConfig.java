...
import com.birdstudio.mars.cluster.directory.StaticDirectory;
import com.birdstudio.mars.cluster.support.AvailableCluster;
import com.birdstudio.mars.cluster.support.ClusterUtils;
import com.birdstudio.mars.config.annotation.Reference;
...
import com.birdstudio.mars.remoting.StaticContext;
import com.birdstudio.mars.remoting.protocol.injvm.InjvmProtocol;
import com.birdstudio.mars.remoting.service.GenericService;
import com.birdstudio.mars.remoting.support.ProtocolUtils;
...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
	...
	private final Object finalizerGuardian = new Object() {
		...
		protected void finalize() throws Throwable {
			...
			if (!ReferenceConfig.this.destroyed) {
				logger.warn("ReferenceConfig(" + url
	...
	private void init() {
		...
		if (interfaceName == null || interfaceName.length() == 0) {
			throw new IllegalStateException(
					"<mars:reference interface=\"\" /> interface not allow null!");
		}
		// 获取消费者全局配置
		checkDefault();
		...
		if (resolve == null || resolve.length() == 0) {
			resolveFile = System.getProperty("mars.resolve.file");
			if (resolveFile == null || resolveFile.length() == 0) {
				File userResolveFile = new File(new File(
						System.getProperty("user.home")),
						"mars-resolve.properties");
		...
		if (resolve != null && resolve.length() > 0) {
			...
			if (logger.isWarnEnabled()) {
				if (resolveFile != null && resolveFile.length() > 0) {
					logger.warn("Using default mars resolve file "
							+ resolveFile + " replace " + interfaceName + ""
		...
		map.put(Constants.MARS_VERSION_KEY, Version.getVersion());
		...
		if (methods != null && methods.size() > 0) {
			for (MethodConfig method : methods) {
				...
				appendAttributes(attributes, method,
						prifix + "." + method.getName());
	...
	private static void checkAndConvertImplicitConfig(MethodConfig method,
			Map<String, String> map, Map<Object, Object> attributes) {
		// check config conflict
		if (Boolean.FALSE.equals(method.isReturn())
				&& (method.getOnreturn() != null || method.getOnthrow() != null)) {
			throw new IllegalStateException(
		...
		String onReturnMethodKey = StaticContext.getKey(map, method.getName(),
				Constants.ON_RETURN_METHOD_KEY);
		Object onReturnMethod = attributes.get(onReturnMethodKey);
		if (onReturnMethod != null && onReturnMethod instanceof String) {
			attributes.put(
					onReturnMethodKey,
					getMethodByName(method.getOnreturn().getClass(),
							onReturnMethod.toString()));
		}
		// convert onthrow methodName to Method
		String onThrowMethodKey = StaticContext.getKey(map, method.getName(),
				Constants.ON_THROW_METHOD_KEY);
		Object onThrowMethod = attributes.get(onThrowMethodKey);
		if (onThrowMethod != null && onThrowMethod instanceof String) {
			attributes.put(
					onThrowMethodKey,
					getMethodByName(method.getOnthrow().getClass(),
							onThrowMethod.toString()));
		}
		// convert oninvoke methodName to Method
		String onInvokeMethodKey = StaticContext.getKey(map, method.getName(),
				Constants.ON_INVOKE_METHOD_KEY);
		Object onInvokeMethod = attributes.get(onInvokeMethodKey);
		if (onInvokeMethod != null && onInvokeMethod instanceof String) {
			attributes.put(
					onInvokeMethodKey,
					getMethodByName(method.getOninvoke().getClass(),
	...
	private T createProxy(Map<String, String> map) {
		...
		if (this.getScope() == "local") {
			...
			isJvmRefer = this.getScope() == "local";
		}
			...
			if (url != null && url.length() > 0) { // 用户指定URL，指定的URL可能是对点对直连地址，也可能是注册中心URL
				...
				if (us != null && us.length > 0) {
					for (String u : us) {
						...
						if (Constants.REGISTRY_PROTOCOL.equals(url
								.getProtocol())) {
				...
				if (urls == null || urls.size() == 0) {
					throw new IllegalStateException(
							"No such any registry to reference "
									...
									+ " use mars version "
									+ Version.getVersion()
									+ ", please config <mars:registry address=\"...\" /> to your spring config.");
				}
			}
		...
		if (c && !invoker.isAvailable()) {
			throw new IllegalStateException(
					"Failed to check the status of the service "
							...
							+ " from the url " + invoker.getUrl()
							+ " to the consumer " + NetUtils.getLocalHost()
							+ " use mars version " + Version.getVersion());
		}
		if (logger.isInfoEnabled()) {
			logger.info("Refer mars service " + interfaceClass.getName()
