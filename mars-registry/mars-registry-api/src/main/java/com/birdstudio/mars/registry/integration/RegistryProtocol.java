 ...
 */
package com.birdstudio.mars.registry.integration;
...
import com.birdstudio.mars.cluster.Configurator;
import com.birdstudio.mars.registry.NotifyListener;
...
import com.birdstudio.mars.remoting.protocol.InvokerWrapper;
...
public class RegistryProtocol implements Protocol {
	...
	public static RegistryProtocol getRegistryProtocol() {
		if (INSTANCE == null) {
			ExtensionLoader.getExtensionLoader(Protocol.class).getExtension(
					Constants.REGISTRY_PROTOCOL); // load
		}
		return INSTANCE;
	}
	...
	private final static Logger logger = LoggerFactory
			.getLogger(RegistryProtocol.class);
	...
	private <T> ExporterChangeableWrapper<T> doLocalExport(
		...
		ExporterChangeableWrapper<T> exporter = (ExporterChangeableWrapper<T>) bounds
				.get(key);
		if (exporter == null) {
			synchronized (bounds) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private <T> void doChangeLocalExport(final Invoker<T> originInvoker,
			URL newInvokerUrl) {
		String key = getCacheKey(originInvoker);
		final ExporterChangeableWrapper<T> exporter = (ExporterChangeableWrapper<T>) bounds
				.get(key);
		if (exporter == null) {
			...
			final Invoker<T> invokerDelegete = new InvokerDelegete<T>(
	...
	private Registry getRegistry(final Invoker<?> originInvoker) {
		...
		if (Constants.REGISTRY_PROTOCOL.equals(registryUrl.getProtocol())) {
			...
			registryUrl = registryUrl.setProtocol(protocol).removeParameter(
	...
	private URL getSubscribedOverrideUrl(URL registedProviderUrl) {
		return registedProviderUrl.setProtocol(Constants.PROVIDER_PROTOCOL)
				.addParameters(Constants.CATEGORY_KEY,
						Constants.CONFIGURATORS_CATEGORY, Constants.CHECK_KEY,
	...
	private URL getProviderUrl(final Invoker<?> origininvoker) {
		String export = origininvoker.getUrl().getParameterAndDecoded(
				Constants.EXPORT_KEY);
		if (export == null || export.length() == 0) {
			throw new IllegalArgumentException(
					"The registry export url is null! registry: "
	...
	private String getCacheKey(final Invoker<?> originInvoker) {
		URL providerUrl = getProviderUrl(originInvoker);
		String key = providerUrl.removeParameters("dynamic", "enabled")
				.toFullString();
		return key;
	}
	...
	public <T> Invoker<T> refer(Class<T> type, URL url) throws RpcException {
		url = url.setProtocol(
				url.getParameter(Constants.REGISTRY_KEY,
						Constants.DEFAULT_REGISTRY)).removeParameter(
				Constants.REGISTRY_KEY);
		...
		// group="a,b" or group="*"
		Map<String, String> qs = StringUtils.parseQueryString(url
	...
	private Cluster getMergeableCluster() {
		return ExtensionLoader.getExtensionLoader(Cluster.class).getExtension(
				"mergeable");
	}
	...
	private <T> Invoker<T> doRefer(Cluster cluster, Registry registry,
		...
		URL subscribeUrl = new URL(Constants.CONSUMER_PROTOCOL,
				NetUtils.getLocalHost(), 0, type.getName(), directory.getUrl()
						.getParameters());
		if (!Constants.ANY_VALUE.equals(url.getServiceInterface())
				&& url.getParameter(Constants.REGISTER_KEY, true)) {
			registry.register(subscribeUrl.addParameters(
	...
	private class OverrideListener implements NotifyListener {
		...
		public void notify(List<URL> urls) {
			...
			List<ExporterChangeableWrapper<?>> exporters = new ArrayList<ExporterChangeableWrapper<?>>(
					bounds.values());
	...
	private class ExporterChangeableWrapper<T> implements Exporter<T> {
		...
		public ExporterChangeableWrapper(Exporter<T> exporter,
