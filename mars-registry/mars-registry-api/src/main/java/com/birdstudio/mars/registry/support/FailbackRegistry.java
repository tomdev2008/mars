...
public abstract class FailbackRegistry extends AbstractRegistry {
    ...
    // 定时任务执行器
    private final ScheduledExecutorService retryExecutor = Executors.newScheduledThreadPool(1, new NamedThreadFactory("MarsRegistryFailedRetryTimer", true));
    ...
    public void subscribe(URL url, NotifyListener listener) {
        ...
        try {
            ...
            if (urls != null && urls.size() > 0) {
                ...
                logger.error("Failed to subscribe " + url + ", Using cached list: " + urls + " from cache file: " + getUrl().getParameter(Constants.FILE_KEY, System.getProperty("user.home") + "/mars-registry-" + url.getHost() + ".cache") + ", cause: " + t.getMessage(), t);
            } else {
                // 如果开启了启动时检测，则直接抛出异常
                boolean check = getUrl().getParameter(Constants.CHECK_KEY, true)
                        && url.getParameter(Constants.CHECK_KEY, true);
