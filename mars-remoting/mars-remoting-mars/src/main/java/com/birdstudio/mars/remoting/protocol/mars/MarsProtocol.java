...
import com.birdstudio.mars.network.exchange.Exchangers;
import com.birdstudio.mars.network.exchange.support.ExchangeHandlerAdapter;
...
import com.birdstudio.mars.remoting.protocol.AbstractProtocol;

/**
 * mars protocol support.
 * 
 */
public class MarsProtocol extends AbstractProtocol {

	public static final String NAME = "mars";

	public static final String COMPATIBLE_CODEC_NAME = "mars1compatible";
	...
	private final ExchangeHandler requestHandler = new ExchangeHandlerAdapter() {
		...
		public Object reply(ExchangeChannel channel, Object message)
			...
			if (message instanceof Invocation) {
				...
				if (Boolean.TRUE.toString().equals(
						inv.getAttachments().get(IS_CALLBACK_SERVICE_INVOKE))) {
					String methodsStr = invoker.getUrl().getParameters()
							.get("methods");
		...
		public void disconnected(Channel channel) throws RemotingException {
			if (logger.isInfoEnabled()) {
				logger.info("disconected from " + channel.getRemoteAddress()
						+ ",url:" + channel.getUrl());
			}
		...
		private Invocation createInvocation(Channel channel, URL url,
				String methodKey) {
	...
	private static MarsProtocol INSTANCE;

	public MarsProtocol() {
		INSTANCE = this;
	}

	public static MarsProtocol getDubboProtocol() {
		if (INSTANCE == null) {
			ExtensionLoader.getExtensionLoader(Protocol.class).getExtension(
					MarsProtocol.NAME); // load
		}
		return INSTANCE;
	}
	...
	Invoker<?> getInvoker(Channel channel, Invocation inv)
		...
		if (isCallBackServiceInvoke) {
			...
			inv.getAttachments().put(IS_CALLBACK_SERVICE_INVOKE,
					Boolean.TRUE.toString());
		}
		String serviceKey = serviceKey(port, path,
				inv.getAttachments().get(Constants.VERSION_KEY), inv
						.getAttachments().get(Constants.GROUP_KEY));

		MarsExporter<?> exporter = (MarsExporter<?>) exporterMap
				.get(serviceKey);

		if (exporter == null)
			throw new RemotingException(channel, "Not found exported service: "
					+ serviceKey + " in " + exporterMap.keySet()
	...
	public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
		...
		MarsExporter<T> exporter = new MarsExporter<T>(invoker, key,
				exporterMap);
		...
		Boolean isCallbackservice = url.getParameter(
				Constants.IS_CALLBACK_SERVICE, false);
		if (isStubSupportEvent && !isCallbackservice) {
			String stubServiceMethods = url
					.getParameter(Constants.STUB_EVENT_METHODS_KEY);
	...
	private ExchangeServer createServer(URL url) {
		...
		url = url
				.addParameter(Constants.CODEC_KEY, Version
						.isCompatibleVersion() ? COMPATIBLE_CODEC_NAME
						: MarsCodec.NAME);
	...
	public <T> Invoker<T> refer(Class<T> serviceType, URL url)
			throws RpcException {
		// create rpc invoker.
		MarsInvoker<T> invoker = new MarsInvoker<T>(serviceType, url,
	...
	private ExchangeClient getSharedClient(URL url) {
		...
		client = new ReferenceCountExchangeClient(exchagneclient,
				ghostClientMap);
	...
	private ExchangeClient initClient(URL url) {
		...
		// client type setting.
		String str = url.getParameter(Constants.CLIENT_KEY, url.getParameter(
				Constants.SERVER_KEY, Constants.DEFAULT_REMOTING_CLIENT));

		String version = url.getParameter(Constants.MARS_VERSION_KEY);
		boolean compatible = (version != null && version.startsWith("1.0."));
		url = url
				.addParameter(
						Constants.CODEC_KEY,
						Version.isCompatibleVersion() && compatible ? COMPATIBLE_CODEC_NAME
								: MarsCodec.NAME);
		// 默认开启heartbeat
		url = url.addParameterIfAbsent(Constants.HEARTBEAT_KEY,
				String.valueOf(Constants.DEFAULT_HEARTBEAT));
		...
		if (str != null
			...
			throw new RpcException(
					"Unsupported client type: "
							...
							+ StringUtils.join(ExtensionLoader
		...
		try {
			...
			throw new RpcException(
					"Fail to create remoting client for service(" + url + "): "
							+ e.getMessage(), e);
		}
		return client;
	}
	...
	public void destroy() {
		...
		for (String key : new ArrayList<String>(referenceClientMap.keySet())) {
			...
			if (client != null) {
				try {
					if (logger.isInfoEnabled()) {
						logger.info("Close mars connect: "
