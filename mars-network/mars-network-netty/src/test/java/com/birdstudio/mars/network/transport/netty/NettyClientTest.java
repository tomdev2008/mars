...
public class NettyClientTest {
	...
	public static void setUp() throws Exception {
		server = Exchangers.bind(
				URL.valueOf("exchange://localhost:10001?server=netty"),
	...
	public void testClientClose() throws Exception {
		...
		for (int i = 0; i < 100; i++) {
			ExchangeChannel client = Exchangers.connect(URL
					.valueOf("exchange://localhost:10001?client=netty"));
			Thread.sleep(5);
			clients.add(client);
		}
	...
	public void testServerClose() throws Exception {
		for (int i = 0; i < 100; i++) {
			Server aServer = Exchangers.bind(
					URL.valueOf("exchange://localhost:" + (5000 + i)
