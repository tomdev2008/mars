...
import com.birdstudio.eirene.utils.LogUtil;
import com.birdstudio.eirene.utils.MarsAppender;
...
import com.birdstudio.mars.network.Server;
...
public class ClientReconnectTest {
	...
	public void testReconnectWarnLog() throws RemotingException,
		...
		// 时间不够长，不会产生error日志
		Assert.assertEquals("no error message ", 0,
				LogUtil.findMessage(Level.ERROR, "client reconnect to "));
		// 第一次重连失败就会有warn日志
		Assert.assertEquals("must have one warn message ", 1,
	 ...
	 */
	@Test
	public void testReconnectErrorLog() throws RemotingException,
			InterruptedException {
		int port = NetUtils.getAvailablePort();
		MarsAppender.doStart();
		String url = "exchange://127.0.0.3:" + port
				+ "/client.reconnect.test?check=false&"
				+ Constants.RECONNECT_KEY + "=" + 1 + // 1ms
														// reconnect,保证有足够频率的重连
		...
		Thread.sleep(1500);// 重连线程的运行
		Assert.assertEquals("only one error message ", 1,
				LogUtil.findMessage(Level.ERROR, "client reconnect to "));
		MarsAppender.doStop();
	}
	 ...
	 */
	@Test
	public void testClientReconnectMethod() throws RemotingException,
			InterruptedException {
		int port = NetUtils.getAvailablePort();
		String url = "exchange://127.0.0.3:" + port
				+ "/client.reconnect.test?check=false&"
		...
		Thread.sleep(1500);// 重连线程的运行
		Assert.assertTrue(
				"have more then one warn msgs . bug was :"
						+ LogUtil.findMessage(Level.WARN,
								"client reconnect to "),
	 ...
	 */
	@Test
	public void testReconnectWaringLog() throws RemotingException,
			InterruptedException {
		int port = NetUtils.getAvailablePort();
		MarsAppender.doStart();
		String url = "exchange://127.0.0.4:" + port
				+ "/client.reconnect.test?check=false&"
				+ Constants.RECONNECT_KEY + "=" + 1 // 1ms reconnect,保证有足够频率的重连
				+ "&" + Constants.SHUTDOWN_TIMEOUT_KEY + "=1"// shutdown时间足够短，确保error日志输出
		...
		Assert.assertTrue("warning message count must >= 1, real :" + count,
				count >= 1);
		MarsAppender.doStop();
	}

	public Client startClient(int port, int reconnectPeriod)
