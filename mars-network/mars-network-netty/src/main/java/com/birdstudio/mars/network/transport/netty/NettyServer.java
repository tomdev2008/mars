...
import com.birdstudio.mars.network.transport.AbstractServer;
...
public class NettyServer extends AbstractServer implements Server {
	...
	protected void doClose() throws Throwable {
		...
		try {
			Collection<com.birdstudio.mars.network.Channel> channels = getChannels();
			if (channels != null && channels.size() > 0) {
				for (com.birdstudio.mars.network.Channel channel : channels) {
					try {
						channel.close();
					} catch (Throwable e) {
						logger.warn(e.getMessage(), e);
