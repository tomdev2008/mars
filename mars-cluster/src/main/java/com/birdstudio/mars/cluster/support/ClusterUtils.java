...
public class ClusterUtils {
	...
	public static URL mergeUrl(URL remoteUrl, Map<String, String> localMap) {
		...
		if (remoteMap != null && remoteMap.size() > 0) {
			// 版本号使用提供者的
			String mars = remoteMap.get(Constants.MARS_VERSION_KEY);
			if (mars != null && mars.length() > 0) {
				map.put(Constants.MARS_VERSION_KEY, mars);
			}
			...
			String remoteListener = remoteMap
					.get(Constants.INVOKER_LISTENER_KEY);
